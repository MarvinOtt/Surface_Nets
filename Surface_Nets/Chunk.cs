            ...
            for (int x = -1; x < 2; ++x)
            {
                for (int z = -1; z < 2; ++z)
                {
                    for (int y = -1; y < 2; ++y)
                    {
                        ...
                        else { topy = 3; bottomy = 0; }

                        if (x == -1) { rightx = Game1.CHUNK_SIZE; leftx = Game1.CHUNK_SIZE - 3; }
                        else if (x == 0) { rightx = Game1.CHUNK_SIZE; leftx = 0; }
                        else { rightx = 3; leftx = 0; }

                        if (z == -1) { backz = Game1.CHUNK_SIZE; frontz = Game1.CHUNK_SIZE - 3; }
                        else if (z == 0) { backz = Game1.CHUNK_SIZE; frontz = 0; }
                        else { backz = 3; frontz = 0; }



                        if (y == -1) { yfac = -Game1.CHUNK_SIZE + 3; }
                        else if (y == 0) { yfac = 3; }
                        else { yfac = Game1.CHUNK_SIZE + 3; }

                        if (x == -1) { xfac = -Game1.CHUNK_SIZE + 3; }
                        else if (x == 0) { xfac = 3; }
                        else { xfac = Game1.CHUNK_SIZE + 3; }

                        if (z == -1) { zfac = -Game1.CHUNK_SIZE + 3; }
                        else if (z == 0) { zfac = 3; }
