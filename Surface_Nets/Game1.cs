...
namespace Surface_Nets
{
    ...
    public class Game1 : Game
    {
        ...
        private static Vector3 camerapos = new Vector3(-166.6f, -286.5f, -48.4f), camerarichtung;
        ...
        public const int LOADING_RADIUS = 15;
        public const int LOADING_HEIGHT = 6;
        ...
        protected override void LoadContent()
        {
            ...
            floor = new Floor(GraphicsDevice, 50, 50, 0, 10);

            world = new World(GraphicsDevice);
            world.InitializeWorld(7);
            world.StartUpdating();
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            if (camerapos.X < 0)
            { currentccameraoffset.X += (int)((-(camerapos.X - CHUNK_SIZE)) / CHUNK_SIZE); camerapos.X += (int)((-(camerapos.X - CHUNK_SIZE)) / CHUNK_SIZE) * CHUNK_SIZE; }
            if (camerapos.Y < 0)
            { currentccameraoffset.Y += (int)((-(camerapos.Y - CHUNK_SIZE)) / CHUNK_SIZE); camerapos.Y += (int)((-(camerapos.Y - CHUNK_SIZE)) / CHUNK_SIZE) * CHUNK_SIZE; }
            if (camerapos.Z < 0)
            { currentccameraoffset.Z += (int)((-(camerapos.Z - CHUNK_SIZE)) / CHUNK_SIZE); camerapos.Z += (int)((-(camerapos.Z - CHUNK_SIZE)) / CHUNK_SIZE) * CHUNK_SIZE; }

            if (camerapos.X >= CHUNK_SIZE)
            { currentccameraoffset.X -= (int)(((camerapos.X)) / CHUNK_SIZE); camerapos.X -= (int)(((camerapos.X)) / CHUNK_SIZE) * CHUNK_SIZE; }
            if (camerapos.Y >= CHUNK_SIZE)
            { currentccameraoffset.Y -= (int)(((camerapos.Y)) / CHUNK_SIZE); camerapos.Y -= (int)(((camerapos.Y)) / CHUNK_SIZE) * CHUNK_SIZE; }
            if (camerapos.Z >= CHUNK_SIZE)
            { currentccameraoffset.Z -= (int)(((camerapos.Z)) / CHUNK_SIZE); camerapos.Z -= (int)(((camerapos.Z)) / CHUNK_SIZE) * CHUNK_SIZE; }
            ...
            cameraspeed = Keyboard.GetState().IsKeyDown(Keys.LeftShift) ? 1.75f : 0.03f;
            ...
            if (camerabewegen == true && this.IsActive)
            {
                ...
                if (changed == 1)
                {
                    Mouse.SetPosition(mousepointpos.X, mousepointpos.Y);
                            ...
                            for (int z = -minesize; z < minesize + 1; ++z)
                            {
                                ...
                                if (length <= radius * radius)
                                {
                                    if (world.CanBeMined(xx - currentccameraoffset.X * CHUNK_SIZE + x, yy - currentccameraoffset.Y * CHUNK_SIZE + y, zz - currentccameraoffset.Z * CHUNK_SIZE + z))
                                    {
                                        ...
                                        int strength = (int)(6750 * MathHelper.Clamp(1 - (length / (radius * radius)), 0.0f, 1.0f));
                                        world.worlddeformer.MineDataAt(pos, strength);
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            int xx = (int)camerapos.X - currentccameraoffset.X * CHUNK_SIZE, yy = (int)camerapos.Y - currentccameraoffset.Y * CHUNK_SIZE, zz = (int)camerapos.Z - currentccameraoffset.Z * CHUNK_SIZE;
